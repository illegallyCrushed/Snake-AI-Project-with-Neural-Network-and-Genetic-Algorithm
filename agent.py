...
class SnakeAgent:
    ...
    def think(self, state):
        ...
        if random.random() <= self.epsilon:
            # if exploitaiton, try random move
            action = random.randint(0, 2)
            final_action[action] = 1
        else:
            # if exploration, calculate step from neural
    ...
    # generate move from brain for given sensor for playing the game
    def play(self, state):
        final_action = [0] * OUTPUT_LAYER
        ts_state = tensorflow.convert_to_tensor([state])
        result = self.brain.neural.predict(ts_state)
    ...
    def trainBatch(self):
        ...
        if len(self.memory) > MAX_BATCH:
            # if memory is more than batch, get random sample
            memory_sample = copy.deepcopy(random.sample(self.memory, MAX_BATCH))
        else:
            # else don't train
            return
        self.trainer.train(memory_sample)
        # if train, reduce epsilon according to decay
        if self.epsilon > EPSILON_MIN:
            self.epsilon *= EPSILON_DECAY

    # save individual snake agent
